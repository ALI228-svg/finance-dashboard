    ...
    <aside className={`hidden md:flex flex-col w-64 min-h-screen px-4 py-6 gap-2 border-r ${dark ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"}`}>
      ...
      <nav className="flex-1 flex flex-col gap-1">
        {nav.map(({ icon: Icon, label, active }) => (
          <button key={label} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${active ? "bg-indigo-500 text-white" : dark ? "text-gray-400 hover:bg-gray-800 hover:text-white" : "text-gray-500 hover:bg-gray-100 hover:text-gray-900"}`}>
            <Icon size={18} />
            {label}
          </button>
        ))}
      </nav>
      <button className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium ${dark ? "text-gray-400 hover:bg-gray-800" : "text-gray-500 hover:bg-gray-100"}`}>
